...
template<size_t Base> BigInt<Base>::BigInt(std::string& s){
  digits_.clear();
  int n = s.size();
  for (int i = n - 1; i >= 0;i--){
...
template<size_t Base> BigInt<Base>::BigInt(long n = 0) {
  do {
...
template<size_t Base> BigInt<Base>::BigInt(const char* n) {
  digits_.clear();
  for (int i = std::strlen(n) - 1; i >= 0; --i) {
    digits_.push_back(n[i] - '0');
  }
}

template<size_t Base> BigInt<Base>::BigInt(const BigInt<Base>& n) {
