...
void QuickSort<T>::quickSortTrace(int start, int end) {
  if (start < end) {
    int pivotIndex = partitionTrace(start, end);
...
template <class T>
int QuickSort<T>::partitionTrace(int start, int end) {
  ...
  for (int j = start; j < end; j++) {
    if (this->data_[j] < pivot) {
      countSwaps++;
      i++;
      std::cout << "swap between " << this->data_[i] << " [i] " << i << " and " << this->data_[j] << " [j] " << j << std::endl;
      T temp = this->data_[i];
  ...
  std::cout << "out 'for' swap:\n"; 
  std::cout << "\tswap between " << this->data_[i + 1] << " and " << this->data_[end] << std::endl;
  T temp = this->data_[i + 1];
  this->data_.set(i + 1, this->data_[end]);
  this->data_.set(end, temp);
  std::cout << "Number of swaps: " << countSwaps << std::endl << std::endl;
